...
use QRcode;
use App\Models\PaymentPixModel;


class ProductWithSicrediController extends Controller
{
	...
	public function bookProductManualy(Request $request)
	{
		...
		public function gerarPIX(ModelsProduct $product, $resultPricePIX, $email, $name, $participante, $cpf, $telefone)
		{
			...
			switch ($product->gateway) {
				case 'mp':
					return $this->handleMercadoPago($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone);
					case 'asaas':
						return $this->handleAsaas($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone);
						case 'paggue':
							return $this->handlePaggue($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone);
							case 'sicredi':
								return $this->handleSicredi($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone);
								default:
	...
	if ($request['action'] == 'payment.updated') {
		...
		$message->from('info@expressaodigital.com', $environment);
	...
	public function notificacoesPaggue(Request $request)
	{
		...
		private function respostaVazia()
		{
		return [
		'codePIXID' => '',
		'codePIX'   => '',
	...
	private function handleMercadoPago($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone)
	{
		...
		"payer" => [
		"email" => $email ? $email : "info@expressaodigital.com",
		"first_name" => $name,
		"identification" => [
			"type" => "hash",
			"number" => date('YmdHis')
	...
	private function handleAsaas($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone)
	{
	...
	private function handlePaggue($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone)
	{
	...
	private function handleSicredi($product, $resultPricePIX, $codeKeyPIX, $email, $name, $participante, $cpf, $telefone)
	{

		$service = new SicrediPixService();
		...
		try {
			$response = $service->generateCharge($data);

			Log::info('Cobrança PIX criada com sucesso', [
				'txid' => $data['txid'],
				'pix_id' => $response['txid'] ?? null,
				'valor' => $data['valor'],
			]);
	 ...
	 public function callbackPaymentSicredi(Request $request)
    {
        // Log para auditoria ou debug
        Log::info('Recebido Webhook do Sicredi', ['payload' => $request->all()]);

        try {
            $data = $request->all();

            if (!isset($data['pix']) || !is_array($data['pix'])) {
                Log::warning('Webhook inválido do Sicredi: campo pix ausente ou incorreto.');
			...
			->where('key_pix', $request['data']['id'])
			->update(['status' => 'Concluída']);
			$payment = PaymentPixModel::where('key_pix', $data['data']['id'])->first();
			...
			Log::info("Sicredi Callback atualizado com sucesso");
	

			return response()->json(['message' => 'Webhook processado com sucesso'], 200);

        } catch (\Exception $e) {
            Log::critical('Erro ao processar Webhook Sicredi', ['error' => $e->getMessage()]);
